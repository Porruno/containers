...
echo "predix.version=16.4.3" >> "$DIRNAME/predix.home.prs"
...
VM_ARGS="-Dmbs.log.custom=com.prosyst.util.log.buffer.BufferedLogger -Dmbs.log.useEventThread=false -Dmbs.log.file.entriesThreshold=0 -Dorg.osgi.framework.bootdelegation=org.bouncycastle.* -Djava.security.egd=file:///dev/urandom -Dpredix.machine.watchdog=$PREDIX_MACHINE_WATCHDOG"
...
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.machineadapter-opcua-16.4.3.jar" ]; then
    OPCUA_KEYSTORE_PATH=security/opcua_keystore.jks
fi
if [ -e "$PREDIX_MACHINE_HOME/machine/bundles/com.ge.dspmicro.opcua-server-16.4.3.jar" ]; then
