...
# Try to shutdown by grabbing the process id. Will only work on shells with ps aux capability.
ps aux > /dev/null
if [ $? -eq 0 ]; then
	writeConsoleLog "Terminating Predix Machine process."
	pmpid=$(ps aux | grep com.prosyst.mbs.impl.framework.Start | grep -v grep | awk '{print $2}')
	if [ "x$pmpid" != "x" ]; then
...
# If it doesn't have ps aux use the shutdown hook, will spawn another JVM
	...
	while true; do
		...
		java -jar "$PREDIX_MACHINE_DATA"/machine/bin/predix/com.ge.dspmicro.shutdown-hook-client-16.4.3.jar
