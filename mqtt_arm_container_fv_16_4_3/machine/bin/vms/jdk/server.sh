...
Execute () {
	...
	# * set classpath
	#GE MODIFIED to remove :$EXTRA_CP from $CP
	if [ -n "$FWAPPCP" ]; then
		CP="-cp $_SERVER:$mCP:../../../lib/framework/fwtime.jar";
	else
		CP="-Xbootclasspath/a:$_SERVER:$mCP -cp ../../../lib/framework/fwtime.jar";
	fi
	# * set boot class path
	...
	#GE MODIFIED to add :"${EXTRA_CP}" and braces in $CP - quoting $EXTRA_CP for possible spaces conflicts with fwtime.jar path
	#GE MODIFIED - quoting for FWPRS done here to avoid premature evaluation
	# * execute
	echo $JAVA $VM_ARGS $XDBG $BOOTCP ${CP}:"${EXTRA_CP}" -Dmbs.prs.name="${FWPRS}" $FEATURES $FWCLEAN $MAIN_CLASS
	if [ -z "$FWDUMPCMD" ]; then
		$JAVA $VM_ARGS $XDBG $BOOTCP ${CP}:"${EXTRA_CP}" -Dmbs.prs.name="${FWPRS}" $FEATURES $FWCLEAN $MAIN_CLASS
	fi
}
