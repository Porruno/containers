...
# Constants 
readonly MACHINE_BUILD_VERSION=16.4.3
...
PrintUsage () {
    ...
    echo ""
    echo "EXAMPLES:"
    echo "   sh DockerizeContainer.sh -m ~/MyPredixMachine"
    echo "   sh DockerizeContainer.sh -m ../ --docker_host default --arch x86_64 --container_name agent --tar_name PredixMachine --http_proxy http://proxy-src.research.ge.com:8080 --https_proxy http://proxy-src.research.ge.com:8080 --no_proxy localhost,127.0.0.1,*.ge.com"
    echo ""
    echo "REQUIRED:"
...
DockerizeMachine () {
    ...
    DOCKER_FILE_NAME=${DOCKER_FILE_PREFIX}-${ARCHITECTURE}
    if [ ! -z "$TAR_NAME" ]; then
        TAR_FILE=${TAR_NAME}-${CONTAINER_NAME}-${ARCHITECTURE}-${MACHINE_BUILD_VERSION}.tar
    else
        TAR_FILE=${MACHINE_FOLDER}-${ARCHITECTURE}.tar
    fi
